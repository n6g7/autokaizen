...
function* createProjectSaga({ boardId, name, currentSprint }) {
  try {
    ...
    const { id: hookId } = yield call(Trello.createWebhook, boardId, process.env.HOOK_URL)
    yield call(rsf.database.update, `projects/${boardId}`, {
      hookId,
      name,
    })
    yield call(rsf.database.update, `sprints/${boardId}/${currentSprint}`, {
      start: Date.now(),
    })
